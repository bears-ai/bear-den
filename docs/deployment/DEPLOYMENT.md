...
| **`bear-letta`** | Letta API (`8283`); **`LETTA_MEMFS_SERVICE_URL`** defaults to **`http://bear-memfs:8285`** (not the literal `local`, which is not a valid URL for Letta’s proxy). Canonical memfs files under `/root/.letta/memfs/repository/…` on **`bear-letta-data`**. |
...
- **`LETTA_MEMFS_SERVICE_URL`** (default `http://bear-memfs:8285` in root [`docker-compose.yaml`](../../docker-compose.yaml)) — Letta’s API proxies `/v1/git/*` to this URL; **Redis** (`bear-redis`) is required for git locks. Data stays on **`bear-letta-data`** (shared with **`bear-memfs`**).  
