...
- **bear_id** / **letta_agent_id** / **role_agent_id** — **`bear_id`** is Den’s **public** identifier for a bear in JSON APIs (**v1:** `bear_id` only; see [PHASE1_DECISIONS.md](PHASE1_DECISIONS.md)). **`letta_agent_id`** is Letta’s internal id for **one** runtime agent; a bear may have **several** (per role). Runtime handoffs use **`role_agent_id`** for the selected role (talk / pair / curate / work / watch). Legacy **`bears.letta_agent_id`** is retired for active routing and remains only as historical migration/repair data until dropped.
