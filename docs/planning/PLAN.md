...
- **bear_id** / **letta_agent_id** — **`bear_id`** is Den’s **public** identifier for a bear in JSON APIs (**v1:** `bear_id` only; see [PHASE1_DECISIONS.md](PHASE1_DECISIONS.md)). **`letta_agent_id`** is Letta’s internal id for **one** runtime agent; a bear may have **several** (per role). Prefer **`bear_id`** in user-facing contracts; when an id is required for Letta APIs, name the **role** (talk / pair / curate / work) in docs, UI, and logs. Legacy **`bears.letta_agent_id`** may duplicate the **talk** agent (or sole pre-migration agent) until migration completes—see [MULTI_AGENT_IMPLEMENTATION_PLAN.md](MULTI_AGENT_IMPLEMENTATION_PLAN.md) Phase 8.5 and Phase 10.
