...
**Context:** Product choices for BEARS Phase 1, aligned with [PHASE1_BOOTSTRAP.md](PHASE1_BOOTSTRAP.md) §17. Decision 4 revised 2026-04-16: no template table; duplicate bear instead. Decision 8 added 2026-04-18: memory dashboard **weight** vs bear-detail **state**. Decision 10 added 2026-04-18: **routines** (Idea 5). Decision 11 added 2026-04-19: **artifacts + Garage**. Decision 6 revised 2026-04-19: **no Open WebUI** — Den Deep Chat only.
...
| 10 | Routines & scheduling (Idea 5) | **Phase 1:** **First-class routines** in Den (DB-backed schedules + UI). Each routine is **assigned to one bear**; tools, policy, and membership **inherit** from that bear (same as interactive chat). **File outputs** → **Garage** artifacts bucket ([artifacts-garage-adr.md](../artifacts-garage-adr.md), [routines-automation-adr.md](../routines-automation-adr.md)). **No** automatic **skill-learning / curator** from unattended routine runs unless explicitly designed later. |
| 11 | Artifacts & Garage (S3) | **Not in Letta:** agent outputs (incl. skills), **human uploads**, routine files → **Garage** **artifacts** bucket; **metadata** includes `conversation_id`, provenance (`agent` vs `human_upload`), bear/user; **GC** for stale objects. **Cabinet** / Outline attachments → **separate bucket** — not artifact GC. Optional future: **promote artifact → Cabinet**. See [artifacts-garage-adr.md](../artifacts-garage-adr.md). |
...
- Artifacts & Garage: [artifacts-garage-adr.md](../artifacts-garage-adr.md)
