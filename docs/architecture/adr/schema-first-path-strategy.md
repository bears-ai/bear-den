...
- role-local notes and raw memory written by bears within their role branch.

If agents choose durable artifact paths directly, path selection becomes part of the agent trust boundary. That creates avoidable risks:
...
Den already owns the domain schema for work plans, handoffs, task approval, runs, and observations. The path strategy should therefore remain conservative and schema-first: agents provide semantic inputs, and Den generates durable artifact paths based on validated schema context.

---

## Decision

BEARS will separate path-like state into three categories and keep durable artifact path generation inside Den schema tools.
...
| Raw role memory/notes | Role branch | `talk/notes/...`, `pair/notes/...`, `work/notes/...` | Role-local writes, path-restricted by MemFS sidecar/hook. |

The important boundary is:

> Agents do not choose durable artifact paths directly. Agents provide semantic inputs; Den chooses the path.
...
Durable artifact paths use a timestamp plus short random or id suffix. They do not use user-provided or agent-provided titles.
...
Title-derived slugs are intentionally avoided because they:
...
- Durable artifact paths become deterministic by tool and role, not by agent choice.
...
- Agents have less direct control over artifact organization.
- Den must own and test a small path-generation utility.
- Existing or future tools that accept arbitrary paths need schema review before being used for durable artifacts.

### Mitigations

- Keep role-local notes available under role branches for raw memory and exploratory notes.
...
Implementation should preserve the core boundary from this ADR: agent input schemas provide semantic fields, never arbitrary durable artifact paths.
