...
If you later route LettaBot through Den: lazy or admin-mapped provisioning, `external_identities` for `(channel, external_id) → user_id`. See [PLAN.md](../planning/PLAN.md) value-add table.
...
**Optional later:** LettaBot → **Den** → Letta (unified identity/policy)—see [PLAN.md](../planning/PLAN.md).
