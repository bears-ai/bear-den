...
The split lets a Bear be conversational, collaborative, reflective, autonomous, and observant without giving every capability to one all-powerful agent.
...
Use these phrases when describing the model:
...
- “`curate` decides what becomes shared memory.”
- “`work` performs approved external tasks.”
- “`watch` listens for external events but does not act on them directly.”
- “`talk` and `pair` are the two synchronous user-facing roles.”
...
- Chat agents directly execute arbitrary autonomous work.
