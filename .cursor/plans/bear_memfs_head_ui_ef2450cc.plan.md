...
**Prerequisite:** Land the **Memory Manager** rename first ([`memory_manager_rename_4b2c1a.md`](memory_manager_rename_4b2c1a.md)): `services/api/`, Docker service `bears-memfs-manager`, default URL `http://bears-memfs-manager:8285`.
...
flowchart LR
  ...
  subgraph network [Docker network]
    Mgr[Memory Manager bears-memfs-manager]
  end
...
- **Use Memory Manager** (compose `bears-memfs-manager`, [`services/api/`](services/api/)): it already owns `MEMFS_BASE` / `find_or_create_repo` and the same paths Letta serves over smart-HTTP.
