...
**Letta + Deep Chat (BEARS Phase 1):** set **`LETTA_API_BASE_URL`** (and **`LETTA_API_KEY`** if your Letta server requires it) for provisioning, agent CRUD, and operator views against the **Letta HTTP API**. Set **`LETTA_CODE_BASE_URL`** (and optional **`LETTA_CODE_API_KEY`**) for **web chat only** — `POST /v1/chat/send` and `/v1/chat/*` use **only** the Letta Code harness URL (`letta server` deployment; see [`../services/letta-code/COOLIFY_DEPLOY.md`](../services/letta-code/COOLIFY_DEPLOY.md)) and do **not** fall back to `LETTA_API_BASE_URL`. The legacy env name **`LETTA_BASE_URL`** is accepted as an alias for `LETTA_API_BASE_URL` with a deprecation warning. See [`.env.example`](.env.example). Deep Chat is vendored under `src/web/assets/deep-chat/` (see [`docs/frontend-development.md`](docs/frontend-development.md#bear-chat-deep-chat)). Periodically smoke-test web chat per [`docs/quickstart.md`](docs/quickstart.md#periodic-integration-smoke-web-chat).
