...
**Letta + Loquix (BEARS Phase 1):** set `LETTA_BASE_URL` (and `LETTA_API_KEY` if your Letta instance requires it) so creating bears in the operator console can provision Letta agents and so signed-in users can chat via **`GET /bear/{slug}`** (per-bear Loquix) or **`GET /app`** (multi-bear picker), both using `POST /v1/chat/send` with session cookies. Loquix is vendored under `src/web/assets/loquix/` (see [`docs/frontend-development.md`](docs/frontend-development.md#bear-chat-loquix)). See [`.env.example`](.env.example).
