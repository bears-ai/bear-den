...
3. With **`RUN_WEB=true`**, set **`CODEPOOL_BASE_URL`** to your [Codepool](../../codepool/README.md) service (for example `http://localhost:3030`) unless you run a **production** build, which defaults to **`http://bear-codepool:3030`** when unset (Docker stack). Den will not start the web server without a non-empty Codepool URL.
4. Run:
