...
//! 2. **Git HTTP sidecar:** Set `LETTA_MEMFS_SERVICE_URL` to the memfs service (e.g. `http://bear-memfs:8285`
//!    in root compose) and `LETTA_MEMFS_LOCAL=1` on codepool. Canonical on-disk memory lives on **Letta’s
//!    volume** (`bear-letta-data`, `~/.letta/memfs/repository/...`);
