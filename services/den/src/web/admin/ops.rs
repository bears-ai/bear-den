...
async fn harness_pool_page(
    ...
    let (codepool_status, codepool_detail, codepool_json_pretty) = if !state.codepool.is_enabled() {
        (
            "disabled",
            "CODEPOOL_BASE_URL is not set — required when RUN_WEB=true.".to_string(),
            String::new(),
        )
    } else {
        match state.codepool.fetch_pool_stats().await {
...
async fn letta_health_json(State(state): State<AppState>) -> Json<LettaHealthJson> {
    if !state.letta.is_enabled() {
        return Json(LettaHealthJson {
            ok: false,
            status: "disabled",
            detail: "LETTA_BASE_URL is not set — provisioning and Letta-backed history are off."
                .to_string(),
        });
    }
...
async fn letta_code_harness_page(
    ...
    let skipped = rows.iter().filter(|r| r.letta_agent_id.is_none()).count();
...
async fn letta_code_harness_yaml_download(
    State(state): State<AppState>,
) -> Result<Response, CustomError> {
    let rows = bears_db::list_letta_code_harness_rows(state.sqlx_pool()).await?;
    let yaml = letta_code_harness::render_letta_code_harness_yaml(
