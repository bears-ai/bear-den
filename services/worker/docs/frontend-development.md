...
4. **Letta stream** — `POST /v1/chat/send` returns SSE; the chat handler uses `connect.handler` + `connect.stream` to parse `data:` JSON lines. It surfaces **`error_message`** (including nested `contents`) as stream-safe **`text` + `role: error`**; streams **`reasoning_message`** into a dedicated **HTML** ai bubble (single-line horizontal scroll, optional Expand for full text — plain text only, no Markdown in that bubble per Deep Chat’s `html` vs `text` tradeoff, see [deep-chat#429](https://github.com/OvidijusParsiunas/deep-chat/issues/429)); then **`assistant_message`** as **`text`** (Markdown-friendly). **Assistant text is accumulated for the whole response** (do not reset the buffer when `id` changes between Codepool/Letta stream chunks, or the UI shows a truncated prefix while history/Letta have the full reply). **`stripSystemReminderBlocksLive`** removes only **closed** `<system-reminder>` blocks while streaming; a final pass on stream end can apply the full strip (incl. partial unclosed tag at EOF). **`overwrite: true`** for each chunk after the first assistant chunk (or when replacing **Thinking**). See `lettaSseHandler` in `bear_chat.html`.
