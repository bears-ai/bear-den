...
let streamFinishedUpstreamError = 0;
...
/** Stream ended without exception but only `error_message` SSE lines (no assistant/reasoning). */
export function recordStreamFinishedUpstreamError(): void {
  streamFinishedUpstreamError += 1;
}
...
export function renderPrometheusText(): string {
  const lines: string[] = [
    ...
    "",
    "# HELP codepool_stream_finished_ok_total Streams that emitted at least one assistant_message or reasoning_message SSE line before end.",
    "# TYPE codepool_stream_finished_ok_total counter",
    `codepool_stream_finished_ok_total ${streamFinishedOk}`,
    "",
    "# HELP codepool_stream_finished_empty_fallback_total Streams with no assistant/reasoning/error output; synthetic error_message was appended.",
    ...
    "",
    "# HELP codepool_stream_finished_upstream_error_total Streams that emitted only error_message lines (no assistant/reasoning).",
    "# TYPE codepool_stream_finished_upstream_error_total counter",
    `codepool_stream_finished_upstream_error_total ${streamFinishedUpstreamError}`,
    "",
