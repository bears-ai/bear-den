 ...
 * Uses `user` + optional `conversation_id` in `metadata` to map to the pool (agent id from metadata.bear_agent_id).
 */
export async function handleOpenAIChatCompletions(
  ...
  const agentId = body.metadata?.bear_agent_id?.trim();
  if (!agentId) {
    res.status(400).json({
      error: {
        message:
          "metadata.bear_agent_id is required (Letta agent id for this bear)",
  ...
  try {
    for await (const msg of pool.streamUserMessage(
      ...
      const line = sdkMessageToSseDataLine(msg);
      if (!line) continue;
      const o = JSON.parse(line) as { message_type?: string; content?: string };
