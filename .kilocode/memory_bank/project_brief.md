...
The system is model-agnostic and self-hosted. **Shared knowledge** is **Cabinet** on **Outline** (human- and **bear**-editable via tools); **Letta** holds per‑**bear** memory (each **bear** is a Letta agent). **Den** provisions bears, **users↔bears** membership (many‑to‑many), and surfaces bears in the **Den chat UI** and **Letta Code** harness. See [`docs/planning/PLAN.md`](../../docs/planning/PLAN.md) and Den / multi-user web in [`docs/architecture/DEN_ARCHITECTURE.md`](../../docs/architecture/DEN_ARCHITECTURE.md).
